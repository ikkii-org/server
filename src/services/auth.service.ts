...
export async function signup(
    ...
    await db
        .insert(wallet)
        .values({ userId: user.id, token: walletKey, availableBalance: "0", lockedBalance: "0" })
        .onConflictDoNothing();
...
export async function login(username: string, password: string): Promise<AuthResult> {
    ...
    if (!user.passwordHash) throw new Error("Account has no password set — use wallet auth");
